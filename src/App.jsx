...
// ============================================================================
// APPLICATION PRINCIPALE - EVENTIX V2 PREMIUM
// ============================================================================
...
const EventixApp = () => {
  ...
  return (
    <div className={`min-h-screen ${theme === 'light' ? 'bg-gradient-to-br from-slate-50 via-white to-slate-100' : 'bg-slate-900'}`}>
...
const DashboardView = () => {
  const stats = [
    { label: 'Prospects', value: '12', change: '+3', icon: Users, color: 'blue' },
    { label: 'Devis En Attente', value: '5', change: '+1', icon: FileText, color: 'amber' },
    { label: 'Contrats Signés', value: '8', change: '+2', icon: Check, color: 'green' },
    { label: 'Revenus Mois', value: '$18.5K', change: '+15%', icon: DollarSign, color: 'emerald' },
  ];
...
const PipelineView = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 auto-cols-max overflow-x-auto pb-4">
        {stages.map(stage => (
          <PipelineColumn key={stage.id} stage={stage} deals={deals[stage.id] || []} />
        ))}
      </div>
    </div>
  );
...
const PipelineColumn = ({ stage, deals }) => {
  ...
  return (
    <div className={`min-w-72 border rounded-xl p-4 ${colorClasses[stage.color]}`}>
      <div className={`${headerColor[stage.color]} rounded-lg px-3 py-2 mb-4 flex items-center justify-between`}>
...
const DealCard = ({ deal }) => (
  <div className="bg-white rounded-lg p-4 border border-slate-200 hover:shadow-md transition cursor-grab active:cursor-grabbing">
...
const CalendarEvent = ({ event }) => {
  ...
  return (
    <div className={`${bgColor} text-xs rounded px-2 py-1 truncate cursor-pointer hover:shadow-sm transition`}>
